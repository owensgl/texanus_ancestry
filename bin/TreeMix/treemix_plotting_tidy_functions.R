...
plot_treemix <- function(obj, plot.nodes = TRUE, plot.migration = TRUE, branch.colour = "grey", label = TRUE, ...) {
  ...
  #Custom label renaming for PET_CAN
  stuff$tips$pop <- as.factor(gsub("PET_CAN", "NIV", stuff$tips$pop))
  stuff$tips$pop <- as.factor(gsub("ANN_CEN", "ANN_NON-TEX", stuff$tips$pop))
